...
for itors in range(ntorsions) :
        ...
        # Adjust the clusters (work with the linkage matrix)
        # Look for the right index in the linkage matrix
        #if len(set(cluster_indices)) < nclusters :
        #        ind = nconfs - nclusters
        #        cluster_indices = numpy.arange(nconfs)
        #        if ind != 0 :
        #                # This means we have more than 3 conformers
        #                # Work through the linkage matrix
        #                for i in range(ind) :
        #                        c_nums = dend[i][:2]
        #                        new_clusterind = ind + i
        #                        for ic in c_nums :
        #                                pass
        #        cluster_indices += 1
