...
pathname = '/home/bulo/Ravi/Molecules/set3/n_hexane/Enumerated60/'
...
print ('Number of heavy atoms: ',len(conformers.trimmed_molecules[0]))
i = 0
ind_n = indices_new[i]
ind_o = index_mat[i][0]
ddists, dtorsions = conformers.get_diffs_for_candidate(conformers.geometries[ind_n])
print ('Differences in distance matrix and torsion angles for ',ind_n,' and ',ind_o)
print ('%8i %20.10f %20.10f'%(i,ddists[ind_o],dtorsions[ind_o]))
...
# Lets fit the structures on top of eachother
#atoms = [i for i,at in enumerate(mol.atoms) if at.symbol!='H']
atoms = [0,1,2,3]
crd = conformers.geometries[ind_n]
crd_new = fit_structure(crd,conformers.geometries[ind_o],atoms)
mol.from_array(conformers.geometries[ind_o])
mol.write('%i.xyz'%(ind_o))
mol.from_array(crd_new)
mol.write('%i.xyz'%(ind_n))
