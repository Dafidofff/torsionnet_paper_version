...
class UniqueConformersAMS (Conformers):
        """
        ...
        *   ``generator``   -- A conformer generator object. Has to be set with :meth:`set_generator`.
        ...
        A simple example of (parallel) use::
            ...
            >>> from crest import UniqueConformersAMS
                ...
                if duplicate is None :
                        ...
                        if self.is_rotamer(duplicate, distance_matrix, dihedral_values, angle_values) :
                                ...
                                for iframe in range(len(self.rotamers[duplicate])) :
                                        if not self.rotamers[duplicate].is_rotamer(iframe, distance_matrix, dihedral_values, angle_values) :
                                                isrot = False
                        if isrot :
                                #self.rotamers[duplicate].add_conformer(coords, energy)
                                self.rotamers[duplicate]._add_if_not_duplicate(coords,energy,mol_trimmed,distance_matrix,dihedral_values,angle_values)
                        else :
        ...
        def set_generator (self, method='rdkit', engine_settings=None, nprocs_per_job=1, energy_threshold=6., nprocs=1) :
                ...
                * ``nprocs``          -- Maximum number of parallel AMS processes
                ...
                Conformers.set_generator(self, method, engine_settings, nprocs_per_job, energy_threshold, nprocs)
        ...
        def read (self, dirname, name='ams', enfilename=None, reorder=True, check_for_duplicates=True) :
                """
        ...
        def write (self, filename='ams', write_rotamers=False, dirname='.') :
                """
        ...
        def __str__ (self) :
                ...
                lines = block.split('\n')
                newlines = [lines[0]+'%20s'%('#Rotamers')]
                for i,line in enumerate(lines[1:]) :
                        if len(line.split())==0 : continue
                        newline = line + '%20i'%(len(self.rotamers[i]))
        ...
        def _add_if_not_duplicate (self, coords, energy, mol_trimmed, distance_matrix, dihedral_values, angle_values) :
                ...
                #self.rotamers.append(Conformers())
                self.rotamers.append(UniqueConformersAMS())
