...
# from conformers import UniqueConformersTFD




def calc_gibbs_norm_values(mol, num_confs = 500):
        mol = Chem.AddHs(mol)

        # run ETKDG num_confs times
        conf_ids = AllChem.EmbedMultipleConfs(mol, numConfs=num_confs)
        res = AllChem.MMFFOptimizeMoleculeConfs(mol)

        # Find minimum energy as E0
        not_converged, energies = np.array(res)[:,0], np.array(res)[:,1]
        E0 = min(energies)
