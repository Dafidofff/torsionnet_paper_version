...
from ..utils import to_np, tensor, mkdir, random_seed
    ...
    def step(self):
        ...
        with torch.no_grad():
            for _ in range(config.rollout_length):
                ...
                print("PRED", prediction['a'])

                # step the environment with the action determined by the prediction
                next_states, rewards, done, info = self.task.step(to_np(prediction['a']))
                ...
                storage.add({
                    ...
                    'm': tensor(1 - done).unsqueeze(-1).to(device)
                    })
