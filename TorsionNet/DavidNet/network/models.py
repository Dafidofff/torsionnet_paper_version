...
class RTGNBatch(torch.nn.Module):
    ...
    def forward(self, obs, states=None, action=None):
        ...
        # Creates one geotmetric batch of multiple observations 
        ...
        # Split recurrent states and forward through actor and critic network
        if states:
            hp, cp, hv, cv = states
            policy_states = (hp.to(device), cp.to(device))
            value_states = (hv.to(device), cv.to(device))
        else:
            policy_states, value_states = None, None

        logits, (hp, cp) = self.actor(obs, policy_states)
        v, (hv, cv) = self.critic(obs, value_states)

        # Create categorical distribution over all torsion angles
        # from outputted logits (p/(1-p))
        ...
        # Create prediction dictionary conataining all information. 
        prediction = {
            'a': action,
            'log_pi_a': log_prob,
            'ent': entropy,
